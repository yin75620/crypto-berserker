...
const (
	...
	//LEAST_VOLUME         = 10
	LEAST_TOTAL_VALUE = 1 //1US
)

//當const 用
var (
	// 數量, 利潤, 加速, 價值(美元計價)
	RANK_S = []float64{PER_ORDER_MAX_VOLUME, 0.0048, -3.0, 280}
	RANK_N = []float64{PER_ORDER_MAX_VOLUME, 0.001, -2.0, 280}
)

var marketMap map[string]int = map[string]int{}
...
func main() {
	...
	infoStr := string(mBitmax.GetAccountInfo())
...
func StartLog() *os.File {
	//path := os.Args[0]
	//rPath := strings.Replace(path, ".exe", ".log", 0)
...
func NewQuote(goalCoin, currentCoin string) Quote {
	...
	if marketName == "USDT/USD" {
		...
		askPair, bidPair = mBitmax.GetAskBidPair(exc.CoinPair{BaseCoin: goalCoin, QuotedCoin: currentCoin}, 1)
	}
...
func stratStrategy() int {
	fuDealFlow := NewDealFlow(BTMX, USDT)
	fbuDealFlow := NewDealFlow(BTMX, BTC, USDT)
	bpuDealFlow := NewDealFlow(BTMX, PAX, USDT)
	//futDealFlow := NewDealFlow(BTMX, USDT, USD)
