...
func (ce *CrossExchange) Start() {
	...
	delayMilliSecond := time.Millisecond * time.Duration(10000)
...
func (ce *CrossExchange) stratStrategy() int64 {
	...
	plus1Price := midPrice + 2*tick1
	minus1Price := midPrice - 2*tick1

	ra := 5000 + 25000*rand.Float64()

	ce.postOrder(exchange, "buy", minus1Price, ra+300*rand.Float64(), futures)
	ce.postOrder(exchange, "sell", plus1Price, ra+300*rand.Float64(), futures)
